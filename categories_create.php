...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($categoryName)) {
        ...
        $stmt = $conn->prepare("SELECT category_id FROM file_categories WHERE category_name = :category_name");
        ...
        if ($stmt->fetch()) {
            ...
            // Insert new category
            $stmt = $conn->prepare("INSERT INTO file_categories (category_name, description, is_active, created_by) VALUES (:category_name, :description, :is_active, :created_by)");
            $stmt->bindParam(':category_name', $categoryName);
    ...
    <div class="flex-1 ml-64 transition-all duration-300" id="mainContent">
        ...
        <main class="p-6">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Create New Category</h2>
                    <p class="text-gray-600 dark:text-gray-400">Add a new file category</p>
                </div>
                <div>
