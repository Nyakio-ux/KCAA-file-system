...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        // Check if department name or code already exists (excluding current department)
        $stmt = $conn->prepare("
            SELECT department_id FROM departments 
            WHERE (department_name = :name OR department_code = :code)
    ...
    <div class="flex-1 ml-64 transition-all duration-300" id="mainContent">
        ...
        <main class="p-6">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Department</h2>
                    <p class="text-gray-600 dark:text-gray-400">Update department information</p>
                </div>
                <div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" 
                                class="w-full px-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-transparent dark:bg-gray-700 dark:text-white"><?php echo htmlspecialchars($department['description'] ?? ''); ?></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="head_user_id" name="head_user_id" 
                                ...
                                <?php foreach ($users as $user): ?>
                                    <option value="<?php echo $user['user_id']; ?>" <?php echo ($department['head_user_id'] == $user['user_id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="flex items-center">
                            <input id="is_active" name="is_active" type="checkbox" <?php echo $department['is_active'] ? 'checked' : ''; ?>
