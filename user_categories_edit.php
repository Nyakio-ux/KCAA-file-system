...
// Fetch category data
$stmt = $conn->prepare("SELECT * FROM user_categories WHERE category_id = :category_id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($categoryName)) {
        ...
        $stmt = $conn->prepare("SELECT category_id FROM user_categories WHERE category_name = :category_name AND category_id != :category_id");
        ...
        if ($stmt->fetch()) {
            ...
            $stmt = $conn->prepare("
                UPDATE user_categories SET 
                    ...
                    can_share_files = :can_share,
                    can_approve_files = :can_approve,
                    can_review_files = :can_review,
                    is_active = :is_active
            ...
            if ($stmt->execute()) {
                $_SESSION['success_message'] = "User category updated successfully!";
                header("Location: user_categories.php");
                exit();
            } else {
    ...
    <div class="flex-1 ml-64 transition-all duration-300" id="mainContent">
        ...
        <main class="p-6">
            <div class="mb-6 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Edit User Category</h2>
                    <p class="text-gray-600 dark:text-gray-400">Update user permissions and access levels</p>
                </div>
                <div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="flex items-center">
                                    <input id="can_upload_files" name="can_upload_files" type="checkbox" <?php echo $category['can_upload_files'] ? 'checked' : ''; ?>
                                ...
                                <div class="flex items-center">
                                    <input id="can_share_files" name="can_share_files" type="checkbox" <?php echo $category['can_share_files'] ? 'checked' : ''; ?>
                                        class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 dark:border-gray-600 rounded dark:bg-gray-700">
                                ...
                                <div class="flex items-center">
                                    <input id="can_approve_files" name="can_approve_files" type="checkbox" <?php echo $category['can_approve_files'] ? 'checked' : ''; ?>
                                        class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 dark:border-gray-600 rounded dark:bg-gray-700">
                                ...
                                <div class="flex items-center">
                                    <input id="can_review_files" name="can_review_files" type="checkbox" <?php echo $category['can_review_files'] ? 'checked' : ''; ?>
                                        class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 dark:border-gray-600 rounded dark:bg-gray-700">
                        ...
                        <div class="pt-4 flex justify-end">
                            <button type="submit" class="px-6 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 flex items-center justify-center">
                                <span id="submitText">Update User Category</span>
